...
class MCTSPlayerMixin(object):
    ...
    def Q(self, position: Position, move: tuple)->float:
    ...
    #@profile
    def suggest_move(self, position: Position, inference=False)->tuple:
        ...
        """Select move"""
        on_board_move_prob = position.predict_to_prob(move_prob)
        ...
        """Get win ratio"""
        player = 'W' if position.to_play else 'B'
    ...
    #@profile
    def suggest_move_mcts(self, position: Position, fixed_depth=True)->np.ndarray:
        ...
        if not self.is_expanded(key):
            logger.debug('Expadning Root Node...')
            move_probs, _ = self.run_many(bulk_extract_features([position]))
            self.expand_node(key, position, move_probs[0])
    ...
    async def tree_search(self, position: Position)->float:
    ...
    async def start_tree_search(self, position: Position)->float:
        ...
        if not self.is_expanded(key):
            ...
            self.expand_node(key, position, move_probs)
            ...
            logger.debug("value: {0:.2f} for position: {1}".format(value, position))
    ...
    """
    @staticmethod
    def counter_key(position: Position)->namedtuple:
        if position is None:
    ...
    def env_action(self, position: Position, action_index: int)->Position:
    ...
    #@profile
    def expand_node(self, key: namedtuple, position: Position, move_probabilities: np.ndarray)->None:
        """Expand leaf node"""
