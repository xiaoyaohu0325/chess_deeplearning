...
class AlphaZeroResNet(ResNet):
    """
    The input features st are processed by a residual tower that consists of a single convolutional block followed by
    ...
    2. Batch normalisation
    3. A rectifier non-linearity
    4. A fully connected linear layer that outputs a vector of size 19^2 + 1 = 362 corresponding to logit probabilities
    ...
    def __init__(self, hps, images, labels, zs, mode):
        ...
        super(AlphaZeroResNet, self).__init__(hps, images, labels, mode)
    ...
    def _tower_loss(self, scope, image_batch, label_batch, z_batch, tower_idx):
        ...
        with tf.variable_scope('init'):
            ...
            x = image_batch
            # A convolution of 256 filters of kernel size 3x3 with stride 1
            x = self._conv('init_conv', x, 3, 119, 256, self._stride_arr(1))
            # Batch normalisation
        ...
        with tf.variable_scope('policy_head'):
            ...
            # size 8*8*73 = 4672 corresponding to logit probabilities
            # for all intersections and the pass move
        ...
        with tf.variable_scope('costs'):
            ...
            tf.summary.scalar('cost_tower_{0}'.format(tower_idx), cost)
            tf.summary.scalar('ce_tower_{0}'.format(tower_idx), ce)
            # scale MSE to [0,1]
            tf.summary.scalar('mse_tower_{0}'.format(tower_idx), mse / 4)
        ...
        with tf.variable_scope('move_acc'):
            ...
            tf.summary.scalar('move_accuracy_tower_{0}'.format(tower_idx), acc)
        ...
        with tf.variable_scope('result_acc'):
            ...
            tf.summary.scalar('result_accuracy_tower_{0}'.format(tower_idx), result_acc)
